...
class multi_VGGPerceptualLoss(torch.nn.Module):
    def __init__(self, lam=1, lam_p=1):
        ...
        self.lam_p = lam_p
        self.dwt = DWT()
    def forward(self, out1, out2, out3, gt1, feature_map, Tout_1, Tout_2, Tout_3, T_feature_map, feature_layers=[2]):
        ...
        l1_s = F.l1_loss(out1, gt1)
        l1_t = F.l1_loss(Tout_1, gt1)
        ...
        for fm in feature_map:
            loss = F.l1_loss(self.dwt(fm), self.dwt(T_feature_map[i]))
            i += 1
            feature_loss += loss
        return loss1+loss2+loss3+40*a*feature_loss            
